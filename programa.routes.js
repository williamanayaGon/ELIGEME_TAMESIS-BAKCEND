...
export default function crearRutasPrograma(prisma) {
    ...
    // -------------------------------------------------------------
    // GET /perfil — composición de la población atendida
    // -------------------------------------------------------------
    router.get('/perfil', async (req, res) => {
        try {
            const scope = scopePacientes(req.auth);
            ...
            if (pacientes.length === 0) {
                return res.json({
                    vacio: true,
                    mensaje: 'No hay personas registradas en el programa.',
                    total: 0
                });
            }
            ...
            // Grupos etarios. El de 60 o más se reporta aparte porque es el
            // corte que usan la mayoría de programas de cuidado.
            ...
            // Diagnósticos: se normaliza para que "EPOC severo" y "EPOC Severo"
            // no cuenten como dos cosas distintas.
            ...
            res.status(500).json({ error: 'No se pudo construir el perfil.' });
        }
    });

    // -------------------------------------------------------------
    // GET /intensidad — frecuencia de visitas y seguimiento
    // -------------------------------------------------------------
    router.get('/intensidad', async (req, res) => {
        try {
            const { desde, hasta } = periodoDe(req.query);
            const scope = scopePacientes(req.auth);
            const scopeHijo = scopePorPaciente(req.auth);
            const umbral = await leerUmbral(prisma, req.auth);
            ...
            for (const p of conVisitas) {
                for (let i = 1; i < p.visits.length; i++) {
                    intervalos.push(
                        (new Date(p.visits[i].date) - new Date(p.visits[i - 1].date)) / 86400000
                    );
                }
            }
            ...
            res.json({
                ...
                visitas: {
                    total: visitas,
                    porPersona: Math.round((visitas / totalPacientes) * 100) / 100
                },
                bitacoras: {
                    total: bitacoras,
                    porPersona: Math.round((bitacoras / totalPacientes) * 100) / 100
                },
            ...
            console.error('❌ Error en intensidad:', error);
            res.status(500).json({ error: 'No se pudo calcular la intensidad.' });
        }
    });

    // -------------------------------------------------------------
    // GET /capacidad — cuidadores, profesionales y relación de carga
    // -------------------------------------------------------------
    router.get('/capacidad', async (req, res) => {
        try {
            ...
            console.error('❌ Error en capacidad:', error);
            res.status(500).json({ error: 'No se pudo calcular la capacidad.' });
        }
    });

    // -------------------------------------------------------------
    // GET /situaciones — hechos verificables, cada uno con su criterio
    // -------------------------------------------------------------
    router.get('/situaciones', async (req, res) => {
        try {
            ...
            console.error('❌ Error en situaciones:', error);
            res.status(500).json({ error: 'No se pudieron calcular las situaciones.' });
        }
    });
    ...
    router.put('/umbral', async (req, res) => {
        try {
            ...
            res.json({
                dias, justificacion,
                definidoPorLaEntidad: true,
                actualizado: guardado.updatedAt
            });
        } catch (error) {
    ...
    router.get('/dossier', async (req, res) => {
        try {
            const { desde, hasta } = periodoDe(req.query);
            const scope = scopePacientes(req.auth);
            const base = `${req.protocol}://${req.get('host')}/api/programa`;
            const cabeceras = { Authorization: req.headers.authorization };

            // Se reutilizan los propios endpoints para que el PDF no pueda
            // divergir de lo que muestra la pantalla.
            const traer = async (ruta) => {
                const r = await fetch(`${base}${ruta}`, { headers: cabeceras });
                return r.ok ? r.json() : null;
            };

            const qs = `?desde=${desde.toISOString()}&hasta=${hasta.toISOString()}`;
            const [perfil, intensidad, capacidad, situaciones] = await Promise.all([
                traer('/perfil'), traer(`/intensidad${qs}`), traer('/capacidad'), traer('/situaciones')
            ]);
            ...
            const reportes = await prisma.financialReport.findMany({
                where: {
                    ...
                    periodoInicio: { lte: hasta }, periodoFin: { gte: desde }
                },
            ...
            const atendidas = perfil?.total ?? 0;
            const costoPorPersona = (reportes.length > 0 && atendidas > 0)
                ? Math.round(ejecutado / atendidas)
                : null;
            ...
            if (!perfil || perfil.vacio) {
                vacio(perfil?.mensaje ?? 'Sin datos.');
            } else {
            ...
            if (!intensidad || intensidad.vacio) {
                vacio(intensidad?.mensaje ?? 'Sin datos.');
            } else {
            ...
            if (!capacidad || capacidad.vacio) {
                vacio(capacidad?.mensaje ?? 'Sin datos.');
            } else {
            ...
            if (reportes.length === 0) {
                ...
                dato('Ejecución total', money(ejecutado));
                for (const [t, v] of Object.entries(porTipo)) dato(`  ${t}`, money(v));
                dato('Costo promedio por persona atendida', costoPorPersona !== null ? money(costoPorPersona) : 'No calculable');
                doc.fillColor('#6b7280').fontSize(8)
                    .text(`Calculado sobre ${reportes.length} reporte(s) enviado(s) y ${atendidas} persona(s) registrada(s).`, { indent: 16 });
            }

            // --- 5. Situaciones ---
            titulo('5. Situaciones detectadas');
            if (!situaciones || situaciones.vacio || situaciones.situaciones.length === 0) {
            ...
            doc.fillColor('#78350f').fontSize(8.5);

            doc.text('QUÉ CUBRE', 52, y + 34);
            doc.fontSize(8).text(
